...
for b in subprocess.check_output(["git","ls-files","-z"], cwd=root).split(b'\0'):
    ...
    try:
        ...
        pass
    rows.append({
        "path":rel,"size_bytes":size,"exec":execf,"git_tracked":"y","git_status":"",
        "lfs_pointer":"y" if rel in lfs else "", "last_commit":last_dt,"last_author":last_author
    })
with out.open('w',newline='',encoding='utf-8') as f:
    w=csv.DictWriter(f,fieldnames=[
        "path","size_bytes","exec","git_tracked","git_status",
        "lfs_pointer","last_commit","last_author"
    ])

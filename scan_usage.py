...
def list_lfs(cwd: Path) -> set[str]:
    try:
        out = subprocess.check_output(
            ["git", "lfs", "ls-files"], cwd=cwd, stderr=subprocess.DEVNULL
        ...
        for fn in filenames:
            ...
            try:
                subprocess.check_output(
                    ["git", "ls-files", "--error-unmatch", rel], cwd=root, stderr=subprocess.DEVNULL
                )
                tracked = "y"
            except Exception:
                tracked = ""
            ...
            files_rows.append(
                {
                    ...
                    "legacy_should_remove": (
                        "y"
                        if fn in LEGACY_REMOVE or rel.startswith("legacy/2_ingest_vector")
                        else ""
                    ),
                }
            )

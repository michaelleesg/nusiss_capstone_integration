...
def extract_prompt_entities(text):
    threat_actors = re.findall(r"(APT\d+|TA\d+|Cozy Bear|Molerats|Gaza Cyber Gang|Arid Viper)", text)
    sectors = re.findall(r"(Media|Government|Energy|Defense|Communication Services)", text)
...
def run_pipeline(input_json_path):
    ...
    for record in records:
        ...
        for v in vectors:
            v["source_url"] = record.get("_id") or record.get("assessment", {}).get("url")
            v["article_date"] = record.get("date_time") or record.get("assessment", {}).get("title")

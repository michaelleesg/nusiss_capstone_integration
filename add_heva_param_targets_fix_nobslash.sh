#!/usr/bin/env bash
# add_heva_param_targets_fix_nobslash.sh
# WSL-safe (no set -euo); idempotent; preserves tabs in Makefile recipes.
# Fix: remove stray "\" in jq program and escape $ for Make (use $$tag/$$since).
...
cp -f "$MAKEFILE" "${MAKEFILE}.bak.heva_param_nobs" 2>/dev/null || true

# Remove prior PARAM block if present
awk -v s="$START_MARK" -v e="$END_MARK" '
  BEGIN{skip=0}
  index($0,s){skip=1; next}
  index($0,e){skip=0; next}
  !skip{print}
' "${MAKEFILE}.bak.heva_param_nobs" > "${MAKEFILE}.tmp"

# Append fresh block (tabs are required in recipes)
...
echo "✅ Fixed qdr-since (no backslashes). Backup at ${MAKEFILE}.bak.heva_param_nobs"
echo "Examples:"
